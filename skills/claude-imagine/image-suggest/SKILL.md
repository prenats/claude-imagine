---
name: image-suggest
description: Analyze project structure and visual needs, then recommend a prioritized list of images to generate. Optionally generates all suggestions using the image generation workflow.
...
For each selected image, apply the full `/image-generate` prompt engineering workflow:
...
**Input:** `/image-suggest a construction equipment reseller — trucks, excavators, Caterpillar, Kubota`
...
Before generating:
- Each prompt goes through full /image-generate 150-250 word engineering
